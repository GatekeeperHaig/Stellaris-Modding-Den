...
def automatedCreationAutobuildAPI(modName="cgm_buildings", addedFolders=[], addedFoldersPriority=[], specialBuildingWeight=10, apiOutFolder="", buildingsIgnoredByBU=[],  fullModName="", priority=False): #if multiple are added  in one category, earlier is higher priority
#AUTOMATED CREATION OF EFFECTS AND TRIGGERS USED FOR AUTOBUILD API
  ...
  for modFolder in addedFolders+addedFoldersPriority:
    if modName!="cgm_planets": #those are written separately. this would mess things up!
      ...
      BUArgV=[apiOutFolder+"/common/buildings/*.txt","../CGM/buildings_script_source/common/buildings/*.txt","--output_folder","../NOTES/api_files/cgm_auto_BU/"+modName, "--custom_mod_name", "CGM - {}: Comp Patch".format(modName),"--make_optional", "--scripted_variables",",".join(variableAllFiles),"--copy_folder_first", apiOutFolder,"--helper_file_list","01", "--skip_building", buildingsIgnoredByBU ]
      if modName in ["alphamod", "gwen"]:
