...
def parse(argv):
  ...
  parser.add_argument('fileNameTarget', help='For every entry in this file, tries to overwrite tagToBeCopied by tagToBeCopied from a same "key" entry from fileNamesSources')
  parser.add_argument('tagToBeCopied')
...
def main(args,unused=0):
  ...
  for fileName in globbedList:
    ...
    for entry in sourceEntries[oldEnd:]:
      try:
        toBeCopied=entry.splitToListIfString(args.tagToBeCopied)
  ...
  for entry in targetEntries:
    ...
    entry.getOrCreate(args.tagToBeCopied)
    entry.replace(args.tagToBeCopied, copyFrom.getOrCreate(args.tagToBeCopied))
